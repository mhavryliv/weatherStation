...
public class single_particles extends PApplet {
...
public void draw () {
  background(11*0.35f, 60*0.35f, 10*0.35f);
  //background(0);
  ...
  windClickCounter = windClickCounter - 0.125f;
  ...
  final float windStrengthWeight = 0.1f;
  ...
  for(int x = 0; x < width; ++x) {
    for(int y = 0; y < height; ++y) {
      ...
      drawWeight = ((1.f - val) * 0.75f + 0.25f) * 255;
...
public boolean isPointTouched(PVector point, float[] arr) {
...
public void handleWindInput(String windDir) {
  windClickCounter++;
  if(windDir.equals("N")) {
    windXComponent = 0.f;
    windYComponent = 1.f;
...
class Particle {
  ...
  public void setMaxLifespan(int val) {
    lifespanMax = val;
  }
  
  public float normalisedLifeSpan() {
    return (float)lifespan / (float)lifespanMax;
  }
  
  public void setNormalisedLifeSpan(float newVal) {
    lifespan = constrain((int)(newVal * lifespanMax), 0, lifespanMax);
    //println("lifespan set to " + lifespan);
  }
  
  public void setStopMovingTillDeath(boolean val) {
  ...
  public void setGravity(PVector newGravity) {
    gravity = newGravity;
  }
  
  public void rebirth(float x, float y) {
    float a = random(TWO_PI);
    float speed = random(0.5f,0.5f); // was 0.5, 4
  ...
  public void update() {
    float lifespanDecrement = stopMovingTillDeath ? 0.75f : 1;
    ...
    if(!stopMovingTillDeath) {
      ...
      part.setTint(color(240*0.75f, 94*0.75f, 27*0.75f, lifespan)); // use orange-red colour      
    }
  }
}
class ParticleSystem {
  ...
  PVector currentGravity = new PVector(0,0.1f);
  ...
  public void addParticlesToTotal(int total) {
  ...
  public void processImpactArray(float[] arr) {
    for (Particle p : particles) {
  ...
  public void updateImpactArray(float[] arr) {
  ...
  public void setEmitter(PVector pos) {
  ...
  public void settings() {  size(1000, 800, P2D);  pixelDensity(displayDensity()); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "single_particles" };
    if (passedArgs != null) {
